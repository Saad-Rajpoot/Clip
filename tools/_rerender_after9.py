...
OUTP = Path("output/_after9")                  # FRESH folder — never overwrite prior AFTERs
...
try:
    ...
    print(f"[after9] run_dir={run_dir}  scenes={len(data.get('scenes', []))}  sha_match={sha_ok}", flush=True)
except Exception as _e:
    print(f"[after9] preflight note: {_e}", flush=True)

try:
    res = P.render_from_script(brief, load_config(), OUTP, keep_work=True)
    print(f"[after9] RENDER OK -> {getattr(res, 'video', None)}  secs={getattr(res, 'video_seconds', None)}", flush=True)
except Exception:
    print("[after9] RENDER FAILED:\n" + traceback.format_exc(), flush=True)
    raise
